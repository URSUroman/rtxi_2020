...
const double RMS_THRESHOLD = 0.6;//0.50;	// in Volts, threshold to start recording
...
const double RMS_TERM_THRES = 0.55;	// in Volts, threshold to stop recording (if under, stops)
...
const char TEMP_FILE[100] = "Template_2.txt"; //"LowerHalfTemp.Det";	// Template File (Raw Data)
...
void DataRecorder::Panel::execute(void) { 
   if(recording && !counter++) {
	...
	if (writeData) {
	    ...
	    ss << current_time; 
	    file_id = ss.str();
	    //file_name = (file_path + "file_" + file_id + ".txt").c_str();
	    file_name = file_path + "file_" + file_id + ".txt";

	    char * file_char = new char[file_name.length() + 1];
	    strcpy(file_char,file_name.c_str());

	    
	    QString filename = QString(reinterpret_cast<char*> (file_char));

	    file_5id = H5Fcreate(filename.toLatin1().constData(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
	    file_5id = H5Fopen(filename.toLatin1().constData(), H5F_ACC_RDWR, H5P_DEFAULT);

	    //dimsf[0]=1;
	    dimsf[0]=CB_ARRAY_SIZE+dScounter+1;
	    dataspace = H5Screate_simple(1, dimsf, NULL); 
	    //double data5[dimsf[0]][dimsf[1]];
	    double * data5;
	    data5 = new double [dimsf[0]];
		...
		data5[CB_ARRAY_SIZE+i]=dataStorage[0][i];
		// cout<<"CB_ARRAY_SIZE+i: "<<CB_ARRAY_SIZE+i<<endl;
		// cout<<"dScounter: "<<dScounter<<endl;
		// cout<<"CB_ARRAY_SIZE: "<<CB_ARRAY_SIZE<<endl;
		// cout<<"dimsf[1]: "<<dimsf[1]<<endl;
	      }
	      //}
	    // else{
	    ...
	    //   }
	      ...
	      cout<<"data5 filled"<<endl;
	      
	      datatype = H5Tcopy(H5T_IEEE_F32LE);
	       ...
	       */
	      dataset = H5Dcreate(file_5id, "Double_Array", datatype, dataspace,
	       ...
	       */
	      status = H5Dwrite(dataset, H5T_IEEE_F32LE, H5S_ALL, H5S_ALL,
				H5P_DEFAULT, data5);

	      cout<<"Data writen"<<endl;
