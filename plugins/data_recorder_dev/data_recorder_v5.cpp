...
const double RMS_THRESHOLD = 0.15;//0.50;	// in Volts, threshold to start recording
...
const double RMS_TERM_THRES = 0.12;//0.1;	// in Volts, threshold to stop recording (if under, stops)
...
// Parameters: Trigger
double corr_coeff_trig = 0.73;	//0.74	// Correlation to Template needed for Detection
...
void DataRecorder::Panel::execute(void) { 
   if(recording && !counter++) {
	...
	// No output. Just record as in SAP
	// if (zapEm) {
