...
DataRecorder::Panel::Panel(QWidget *parent, size_t buffersize) :
  QWidget(parent), RT::Thread(RT::Thread::MinimumPriority), fifo(buffersize), fifo_song(buffersize),  recording(false), IO::Block("Connect",chans,num_chans)
{
    setWhatsThis(
...
DataRecorder::Panel::~Panel(void)
{
    ...
    //Added to delete fifo_song during Panel destruction
    delete &fifo_song;
}
...
const double RMS_THRESHOLD = 0.2;//0.90;	// in Volts, threshold to start recording
...
const double RMS_TERM_THRES = 0.1;//.85;	// in Volts, threshold to stop recording (if under, stops)
...
const char TEMP_FILE[45] = "Template_2.txt"; //"LowerHalfTemp.Det";	// Template File (Raw Data)
...
void Compare_Spects()
{
	...
	// Normalize 
	correlation_coefficient = 0;//correlation_coefficient / spectLen;
...
void DataRecorder::Panel::execute(void) { 
   if(recording && !counter++) {
	...
	//Problem here: in v0 there seems to be no problem with sampling rate but in v4
	//there is a pb with sampl rate: the smpl rate sometimes goes up, roughly x2
	// Write Data
	if (writeData) {
		// if (isInteresting) {
		// 	double appendData[2];
	  ...
	  //try to write data as in v0.
			...
			token.type = _START;
			token.size = channels.size() * sizeof(double);
			fifo_song.write(&token, sizeof(token));
			...
			for (int i = 0; i != CB_ARRAY_SIZE; i++) {
			  appendData[0] = predata[0][i];
			  //appendData[1] = predata[1][i];
			  //cout<<"fifo.tooBig for predata: "<<fifo.tooBig(sizeof(token))<<endl;
			  if (!fifo_song.tooBig(sizeof(token)) && !fifo_song.tooBig(token.size)) {
			    fifo_song.write(&token, sizeof(token));
			    fifo_song.write(appendData, sizeof(appendData));
			  }
			  else
			    {
			      cout<<"fifo_song.tooBig for predata: "<<endl;
			    }
			       
			  //cout<<"token _song written exec: "<<token.type<<endl;
			  //Put data to original fifo once in a second
			  h5_counter++;
			...
			for (int i = 0; i != dScounter; i++) {
			  appendData[0] = dataStorage[0][i];
			  //appendData[1] = dataStorage[1][i];
			  //cout<<"fifo.tooBig for dataStorage: "<<fifo.tooBig(sizeof(token))<<endl;
			  if (!fifo_song.tooBig(sizeof(token)) && !fifo_song.tooBig(token.size)) {
			    fifo_song.write(&token, sizeof(token));
			    fifo_song.write(appendData, sizeof(appendData));
			  }
			    ...
			    {
			      cout<<"fifo_song.tooBig for dataStorage: "<<endl;
			    }

			  //cout<<"token _song written exec: "<<token.type<<endl;
			  //cout<<"size_of(append Data): "<<sizeof(appendData)<<endl;
...
void DataRecorder::Panel::processData(void)
{
    ...
    for (;;)
	  ...
	  if(!tokenRetrieved)
	    ...
	    if(!tokenRetrieved_song)
	      {
		// Returns true if data was available and retrieved
		if(fifo_song.read(&_token_song, sizeof(_token_song)))
		  {
		    tokenRetrieved_song = true;
		    cout<<"token _song: "<<_token_song.type<<endl;
		  }
		else
		  {
		    // Sleep loop then restart if no token was retrieved
		    nanosleep(&sleep, NULL);
		    //Don't disturb loop of original fifo
		    //continue;
		  }
	      }
	    ...
	    if(!tokenRetrieved && !tokenRetrieved_song)
	      continue;
	    //**********************************************************************************
	    //*                   First if condition on _token_song                            *
	    //**********************************************************************************
            if (_token_song.type == _START)
	      {
		if (state == RECORD)
		  {
            ...
            else if (_token_song.type == _STOP)
	      {
		if (state == RECORD)
		  {
		    file_to_write.close();
            ...
            else if (_token_song.type == SYNC)
	      {
		if (state == RECORD)
		  {
		    double data[_token_song.size / sizeof(double)];
		    //cout<<"check data in fifo"<<endl;
		    //cout<<"h5_counter: "<<h5_counter<<endl;
		    //cout<<"_token_song.size in SYNC REC: "<<_token_song.size<<endl;
		    if(fifo_song.read(data, _token_song.size))
		      {
			//Don't disturb loop of the original fifo
			...
			if(file_to_write.is_open()){
			  ...
			  {
			    cout<<"File is not opened in SYNC/RECORD. File is open:  "<<file_to_write.is_open()<<endl;
			    if (getcwd(cwd, sizeof(cwd)) != NULL) {
			      cout<<"Current working dir: "<<cwd<<endl;
			    }
			  }
			cout<<"Data written"<<endl;
		      ...
		      {
			cout<<"Not enough data in fifo_song"<<endl;
		      }
		  }
	      }
	    ...
	    //**********************************************************************************
	    //*                   Second if condition on original _token                       *
	    //**********************************************************************************
            if (_token.type == SYNC)
	      {
