...
// Parameters: Trigger
double corr_coeff_trig = 0.76;//0.74;		// Correlation to Template needed for Detection
...
//*******************************************
double syl_th_duration = 5000;//corresponds to the nb of samples at 30303 Hz.
...
void Initialize()
{
	...
	//Do filtering of data from file
	//Extract song from file
	int i=0;
	int size_song=0;
	const char SF_1[45] = "Song_file1.txt"; //"LowerHalfTemp.Det";	// Template File (Raw Data)
	int SONG_LENGTH = 40; //in seconds
	const int SONG_ARRAY_SIZE = SONG_LENGTH * SAMP_FREQ;
	double song_array[SONG_ARRAY_SIZE];
	double song_array_flt[SONG_ARRAY_SIZE];
	double song_array_flt_sqrd[SONG_ARRAY_SIZE];
	double song_array_flt_sqrd_flt[SONG_ARRAY_SIZE];
	ifstream sfile;
	sfile.open(SF_1);
	if(sfile.is_open()){
	  while(!sfile.eof()){
	...
	for(int j = 0;j!=SONG_ARRAY_SIZE;j++){
	  data_flt = 0;
	  if ((j >= (FILTER_SIZE_B-1))) {
	    for (int i = 0; i != FILTER_SIZE_B; i++)
	      data_flt = data_flt + song_array[j-i]*filter_b[i];
	  }
	  else {
	    for (int i = 0; i != (j+1); i++)
	      data_flt = data_flt + song_array[j-i]*filter_b[i];
	  }
	  song_array_flt[j] = data_flt;
	  //Square the bp filtered signal
	  song_array_flt_sqrd[j] = song_array_flt_sqrd*song_array_flt_sqrd;
	}


        //High pass filter the sqared(bp_filtered)) signal
	for(int j = 0;j!=SONG_ARRAY_SIZE;j++){
	  data_flt = 0;
	  if ((j >= (FILTER_SIZE_H-1))) {
	    for (int i = 0; i != FILTER_SIZE_H; i++)
	      data_flt = data_flt + song_array_flt_sqrd[j-i]*filter_h[i];
	  }
	  else {
	    for (int i = 0; i != (j+1); i++)
	      data_flt = data_flt + song_array_flt_sqrd[j-i]*filter_h[i];
	  }
	  song_array_flt_sqrd_flt[j] = data_flt;
	}

	//Wrtie to file the filtered song...........
	...
	if(( circularBuffer_flt[cBcounter]> Amp_th))
	...
	else
	  {
	    if(time_counter > 0) //A syllable has just finished. time_counter contains the duration of the syllable
	      {
		//Check that the silmence has a minimum duration of min_dur_sil
