...
const int SAMP_FREQ = 20 * 1000; 	// in Hertz
...
const double RMS_THRESHOLD = 0.0007;//0.07//0.90;	// in Volts, threshold to start recording
...
const double RMS_TERM_THRES = 0.0007;//0.05;//.85;	// in Volts, threshold to stop recording (if under, stops)
...
const char TEMP_FILE[45] = "I_20_05_2020_20k_124500_126500.txt";//Template_2.txt"; //"LowerHalfTemp.Det";	// Template File (Raw Data)
const int MS_IN_SPECT = 50; //Template_2 with 32 ms		// in ms
const int SPECT_PERIOD = 1000;		// in us

// Parameters: Trigger
double corr_coeff_trig = 0.57;//0.74;		// Correlation to Template needed for Detection
...
string file_path_flt = "../../Documents/Recordings/Song/Raw_Recordings_Impl_flt/";
string file_path_means_thresholds = "../../Documents/Recordings/Song/Means_and_Thresholds_Impl/";
...
double syl_th_duration = 3859;//corresponds to the nb of samples at 30303 Hz.
double syl_and_gap_th_duration = 2500;
const int Nb_syllables = 200;//200
double syllables_durations[Nb_syllables+10] = {0};
int syl_counter = 0;
int syl_counter_no_CAF=0;
int syl_counter_no_CAF_limit=1000110;//ideally we will gather 110 syllables during NO_CAF mode each day for off-line processing 
...
void Initialize()
{
	...
	last_time=(double)current_time_clean_song-0*60*60;//-60*60;
...
void Command_Code(double dataCh1, double dataCh2)
{
	...
	*/
	...
	case MIN_SYL_DUR:
	  if(( circularBuffer_flt[cBcounter]> Amp_th)){
	    ...
	    //cout<<STATE<<endl;
	    min_dur_syl_count=0;
	    time_counter=0;//time_counter counts the length of the (syllable+gap)
	  }
	  break;
	case FIRST_SYL:
	  if(( circularBuffer_flt[cBcounter]> Amp_th)){
	...
	case MIN_PAUSE_DUR:
	  ...
	  else{//Aplitude reaches threshold: the silence was short break within a syllable
	    min_dur_sil_count=0;
	    time_counter++;
	    STATE=FIRST_SYL;
	    //cout<<STATE<<endl;
	  ...
	  if(( circularBuffer_flt[cBcounter]> Amp_th)){
		...
		case CAF:
		  ...
		  if((time_counter<(mean_syl_dur+5*std_dev_syl_dur))&&(time_counter>(mean_syl_dur-5*std_dev_syl_dur))){
		    ...
		    syl_counter++;
		    if(time_counter < (syl_and_gap_th_duration+1000)){//in case the last syll is not sung, avoid noise at the begining of the next bout especially if next bout comes after long time (the bird may be scared and the neuron may be lost)
		      if((rand() % 100)>50){
			zapEm = true;//release noise on 50% of cases
		      }
		    }
	...
	*/
	...
	//   }
	//*******************************************************************************
	//        Comment lines above if you want to use the state machine above above
	//*******************************************************************************
	...
	*/
	...
	if(syl_counter==Nb_syllables){
	  ...
	  if((fract_escape>treshold_fract_escape)&&(mean_syl_dur<syl_and_gap_th_duration)){
	...
	*/
	...
	case CAF:
	  ...
	  if((((double)current_time_clean_song)-last_time)>one_day_delay){
	    ...
	    //*******************************************
	    //change state, uncomment the 2 lines below if want to record without caf
	    STATE_CAF=NO_CAF;
	...
	case NO_CAF:
	  ...
	  if (writeData) {
	    ...
	    if (isInteresting) {
	      ...
	      token.type = _START;
	      token.size = 3 * channels.size() * sizeof(double); //Data point and Data_flt point and dataStorage_corr are doubles . channels.size is normally 1, that's why we have 2*channels.size
	      //token.size = channels.size() * sizeof(double);
	      fifo.write(&token, sizeof(token));
