...
const double RMS_THRESHOLD = 0.06;//0.15;0.90;	// in Volts, threshold to start recording
...
const double RMS_TERM_THRES = 0.02;//0.05;//.85;	// in Volts, threshold to stop recording (if under, stops)
...
const char TEMP_FILE[45] = "Seventh_Bird_20k_108900_110900.txt";//"Sixth_Bird_20k_128772_130772.txt";//"Sixth_Bird_20k_142786_144786.txt";//"Fifth_Bird_20k_64000_66000.txt"; //"Gris1Vert2_20k_70250_72250.txt";//"Gris1Vert2_20k_106700_108000.txt";//"Gris1Vert2_20k_62500_64500.txt";//"Second_bird_syl_a_1.txt";//"Template_2.txt"; //"LowerHalfTemp.Det";	// Template File (Raw Data)
const int MS_IN_SPECT = 50; //Template_2 with 32 ms		// in ms
const int SPECT_PERIOD = 1000;		// in us

// Parameters: Trigger
double corr_coeff_trig = 0.325;//0.62;// (for sixth bird);//0.6 for bird3//0.62//0.74; 0.54 for Fifth_bird		// Correlation to Template needed for Detection

// Parameters: Output
const int OUTPUT_LENGTH = 200; //32;		// in ms; output file length
...
//********************************************
double Amp_th = 4.6e-4;//1e-3 (for sixth bird) ;//5e-4(for fifth bird);//6e-4;//2.5e-3;//2e-5; Fifth Bird: 5e-4 
...
double treshold_fract_escape_low_limit = 0.10;
double syl_th_duration = 10000;//corresponds to the nb of samples at 30303 Hz.
double syl_and_gap_th_duration = 537;//gap only;//3693(for sith bird syllable and gap);//3856; //3731; 849
...
void Initialize()
{
	...
	last_time=(double)current_time_clean_song-6.7*60*60;//-60*60;
	...
	case MIN_SYL_DUR_END:
	  if(( circularBuffer_flt[cBcounter]> Amp_th)){
		...
		case CAF:
		  ...
		  if((pause_counter<(mean_syl_dur+5*std_dev_syl_dur))&&(pause_counter>(mean_syl_dur-5*std_dev_syl_dur))){
		    syllables_durations[syl_counter]=pause_counter;
		    syl_counter++;
		    if(pause_counter < syl_and_gap_th_duration){
	...
	*/
	...
	if(syl_counter==Nb_syllables){
	  ...
	  if((fract_escape>treshold_fract_escape)&&(mean_syl_dur>syl_and_gap_th_duration)){ //mean_syl_dur is actually the duration of the prev_syllable+following gap i.e. is the duration of the target element
	    ...
	    syl_and_gap_th_duration=syl_and_gap_th_duration-0.8*(syl_and_gap_th_duration-mean_syl_dur);
