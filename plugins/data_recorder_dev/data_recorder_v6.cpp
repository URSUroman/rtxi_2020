...
const double RMS_THRESHOLD = 0.9;	// in Volts, threshold to start recording
...
const double RMS_TERM_THRES = .85;	// in Volts, threshold to stop recording (if under, stops)
...
int DataRecorder::Panel::startRecording(long long timestamp)
{
    ...
    //Added :: to ispunct at line 2026 below so that ispunct is taken from the main namespace instead of std namespace.
    size_t count = 0;
    for (RT::List<Channel>::iterator i = channels.begin(), end = channels.end(); i != end; ++i)
        {
