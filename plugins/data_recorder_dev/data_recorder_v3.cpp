...
{
    //Added: IO::Block("Connect",chans,num_chans) in the line above
    setWhatsThis(
...
const double RMS_THRESHOLD = 0.5;      //0.5;	// in Volts, threshold to start recording
...
const double RMS_TERM_THRES = 0.4;      //0.4;	// in Volts, threshold to stop recording (if under, stops)

// Parameters: FFT
const int FFT_ARRAY_SIZE = 256; //256; 	// Must be power of 2
...
const char TEMP_FILE[25] = "Template_2.txt"; //"LowerHalfTemp.Det";	// Template File (Raw Data)
const int MS_IN_SPECT = 55;  //32;		// in ms
const int SPECT_PERIOD = 1000;		// in us

// Parameters: Trigger
double corr_coeff_trig = 0.73;		// Correlation to Template needed for Detection 0.74 initially

// Parameters: Output
const int OUTPUT_LENGTH = 100;//32;		// in ms; output file length
const char PLAYBACK_FILE[20] = "Noise.txt"; // "thisSyl"; // File for Feedback
const int DEAD_TIME = 120; 		// in ms. Default: OUTPUT_LENGTH + 20
...
double circularBuffer_filtd[CB_ARRAY_SIZE];
...
time_t current_time;
string file_path = "../Documents/Recordings/";
string file_name;
stringstream ss;
string file_id;
//ofstream file;
...
double filter_sz = double(FILTER_SIZE);
double filter[FILTER_SIZE];
double filtered_point = 0;
//********************************************
...
//*******************************************
double syl_th_duration = 4761.3;//4697 corresponds to the nb of samples in 155 ms at 30303 Hz. //4454 corresponds to the nb of samples in 147 ms at 30303 Hz 
...
void Initialize()
{
	...
	for (int i = 0; i != FILTER_SIZE; i++)
	  filter[i] = 1.0/double(FILTER_SIZE);
...
void Command_Code(double dataCh1, double dataCh2)
{
	...
	circularBuffer_sqrd[cBcounter] = dataCh1*dataCh1;
	//cout<<"cBcounter: "<<cBcounter<<"\n";

	//******************************************************
	//Filter data (CAN do more efficiently!!!)
	//Naive version
	//******************************************************
	/*
	filtered_point = 0;
	if ((cBcounter >= FILTER_SIZE)) {
	  for (int i = 0; i != FILTER_SIZE; i++)
	    filtered_point = filtered_point + circularBuffer_sqrd[cBcounter-i];
		...
		for (int i = 0; i != cBcounter; i++)
		  filtered_point = filtered_point + circularBuffer_sqrd[cBcounter-i];
		  //filtered_point = filtered_point + circularBuffer_sqrd[cBcounter-i]*filter[i];
		for (int i = 0; i != (FILTER_SIZE-cBcounter); i++)
		  filtered_point = filtered_point + circularBuffer_sqrd[CB_ARRAY_SIZE-1-i];
	...
	circularBuffer_filtd[cBcounter] = filtered_point/filter_sz;
	*/
	...
	//******************************************************
	//Filter data
	//Better version: add newest data point and remove oldest data point
	//******************************************************

	if ((cBcounter >= FILTER_SIZE)) {
	  filtered_point = filtered_point + (circularBuffer_sqrd[cBcounter]-circularBuffer_sqrd[cBcounter-FILTER_SIZE])/filter_sz;
	}
	else {
	  filtered_point = filtered_point + (circularBuffer_sqrd[cBcounter]-circularBuffer_sqrd[CB_ARRAY_SIZE - FILTER_SIZE + cBcounter])/filter_sz;
	}



	//Check whether the amplitude is above Amp_th. If errors will be generated, execute this only each ms or 500 µs
	if((circularBuffer_filtd[cBcounter] > Amp_th))
	  {
	    time_counter++;
	  }
	else
	  {
	    if(time_counter > 0) //A syllable has just finished. time_counter contains the duration of the syllable
	      {
		if(pre_zapEm) //The syllable was the right one
		  {
		    //Record the syllable duration
		    syllables_durations[syl_counter] = time_counter;
		    syl_counter++;
		    if((time_counter < syl_th_duration) && (zapEm == false)) //The syllable is shorther than a threshold
		      {
			//zapEm = true; //Allow noise output
		      }
		    pre_zapEm = false;
		  }
	      }
	  ...
	  if ((  cBcounter % (SAMP_FREQ / (1000000 / RMS_PERIOD)) == ( (SAMP_FREQ / (1000000 / RMS_PERIOD)) - 1) )){
			if ((RmsValue(POINTS_IN_RMS) >= RMS_THRESHOLD)) {
				...
				for (int i = 0, j = cBcounter+1; i < CB_ARRAY_SIZE; i++, j++) {
					...
					//******************************************************
					//Uncomment line below when saving filtered signal
					//******************************************************
					//predata[1][i] = circularBuffer_filtd[j];
	...
	if (rmsHit) {
		if ((record_counter++ == RECORD_MAX)) {
			if ((RmsValue(POINTS_IN_RMS_TERM) < RMS_TERM_THRES)) {
				rmsHit = false;
				cout<<"RMShit reset after the above computation"<<"\n";
				record_counter = 0;
				writeData = true;
...
void Compare_Spects()
{
	...
	if(spectLen != 0){
	  correlation_coefficient = correlation_coefficient / spectLen;
	  //printf("spectLen different from 0 \n",correlation_coefficient);
	}
...
//Test playback on amplitude
void playback_on_amplitude()
{
  const int array_size_for_amplitude = 100;
  double array_for_amplitude[array_size_for_amplitude];
  double mean_amplitude = 0;
  double amplitude_threshold = 0.08;
  int k = 0;
  if ((cBcounter >= array_size_for_amplitude)) {
        for (int i = cBcounter - array_size_for_amplitude; i != cBcounter; i++, k++)
		 array_for_amplitude[k] = circularBuffer[0][i];
  }
  else {
	for (int i = CB_ARRAY_SIZE - (array_size_for_amplitude - cBcounter); i != CB_ARRAY_SIZE; i++, k++)
		 array_for_amplitude[k] = circularBuffer[0][i];
	for (int i = 0; k != array_size_for_amplitude; i++, k++)
		 array_for_amplitude[k] = circularBuffer[0][i];
  }

  for(int i = 0; i < array_size_for_amplitude; i++)
    {
      mean_amplitude = mean_amplitude + abs (array_for_amplitude[i]);
    }
  
  mean_amplitude = mean_amplitude/array_size_for_amplitude;
...
void DataRecorder::Panel::execute(void) {
  ...
  //   cout<<"execute"<<endl;
    ...
    if (writeData) {
      ...
      // 	//DataRecorder::Panel::AppendMe(appendData);
      // 	file << dataStorage[0][i]<<'\n';
      // }
      cout << " write data  Trial " << trialNum << " Complete.\n\n"; // open RTXI from the terminal to see cout messages
      //StopStart();

      //******************************************************
      //Uncomment to write syllable surations (will not be needed since duration will be calculated in python after syllable labelling)
      //*******************************************************
      // //Set mark to sparate amplitude data from syllables durations
      // file << -100 <<'\n';
      
      // //Syllables durations
      // int i = 0;
      // while(syllables_durations[i]>0)
      // 	{
      // 	  file << syllables_durations[i] << '\n';
      // 	  syllables_durations[i] = 0;
      // 	  i++;
      // 	}
      //******************************************************
      //Stop above uncomment
      //******************************************************
      syl_counter=0;
      ss.str(string());
      file.close();
      ...
      //***************************************************
      //Uncomment lines below when saving filtered data + comment ss.str(string()); above
      //***************************************************
      //
      //      //Write signal squared and filtered
      //      file_name = file_path + "file_fltd_" + file_id + ".txt";
      //      file.open(file_name);
      //
      //      //double appendData[2];
      //      for (int i = 0; i != CB_ARRAY_SIZE; i++) {
      ...
      //
      //**************************************************************************
      //Uncomment lines below when saving syllables durations in a separate file
      //**************************************************************************
      //     //Write the syllables durations to file
      //     if(syl_counter > Nb_syllables/50)
      //	{
      //	  time(&current_time);
      //	  ss << current_time; 
      //	  file_id = ss.str();
      //	  file_name = file_path + "file_syllables_durations" + file_id + ".txt";
      //	  file.open (file_name);
      //
      //	  int i = 0;
      //	  while(syllables_durations[i]>0)
    ...
    if (rmsHit && (dScounter < MFS_PTS)) {
      ...
      //***************************************************
      //Uncomment lines below when saving filtered data and comment dScounter++ above
      //***************************************************
      //
      //      if(cBcounter == 0)
      //	{
      //	  dataStorage[1][dScounter++] =  circularBuffer_filtd[CB_ARRAY_SIZE-1];
      //	}
      //      else
      //	{
      //	  dataStorage[1][dScounter++] =  circularBuffer_filtd[cBcounter-1];
      //	}
      //
    ...
    for (RT::List<Channel>::iterator i = channels.begin(), end = channels.end(); i != end; ++i)
        {
            ...
            rec_chan_name.erase(std::remove_if(rec_chan_name.begin(), rec_chan_name.end(), &::ispunct), rec_chan_name.end()); //Modified from &ispunct to &::ispunct
