...
const double RMS_THRESHOLD = 0.2;//0.50;	// in Volts, threshold to start recording
...
const double RMS_TERM_THRES = 0.15;//0.1;	// in Volts, threshold to stop recording (if under, stops)
...
const char TEMP_FILE_1[100] = "Template_1.txt"; //"LowerHalfTemp.Det";	// Template File (Raw Data). Usually, template_1 is the pre-template file
const char TEMP_FILE_2[100] = "Template_2.txt"; //"LowerHalfTemp.Det";	// Template File (Raw Data)
const int MS_IN_SPECT_1 = 32;		// in ms
const int MS_IN_SPECT_2 = 32;		// in ms
const int SPECT_PERIOD = 1000;		// in us
double DTmin = 10; //in ms. Time to wait before the relevant syllable comes after the pre-syllable
double DTmax = 50; //in ms. Max time to wait for the relevant syllables. After that time, look for template_1 anew

// Parameters: Trigger
double corr_coeff_trig_1 = 0.73;	//0.74	// Correlation to Template_1 needed for Detection
double corr_coeff_trig_2 = 0.73;	//0.74	// Correlation to Template_2 needed for Detection

// Parameters: Output
const int OUTPUT_LENGTH = 100; //32;		// in ms; output file length
const char PLAYBACK_FILE[100] = "Noise.txt"; //"thisSyl"; // File for Feedback
...
double spectMean_1[SPECT_ARRAY_SIZE];
double temp_spect_1[SPECT_ARRAY_SIZE][FFT_ARRAY_SIZE/2];
double tempLen_1;

double spectMean_2[SPECT_ARRAY_SIZE];
double temp_spect_2[SPECT_ARRAY_SIZE][FFT_ARRAY_SIZE/2];
double tempLen_2;

int temp_counter = 0;
int which_temp = 1; //1 or 2. Which template to search in the audio rec
...
//PATH_MAX already defined in linux
char cwd[PATH_MAX];
// Functions
void Initialize();
...
void Initialize()
{
	...
	const int TD_SIZE_1 = FFT_ARRAY_SIZE + int(SAMP_FREQ / 1000) * MS_IN_SPECT_1;
	const int TD_SIZE_2 = FFT_ARRAY_SIZE + int(SAMP_FREQ / 1000) * MS_IN_SPECT_2;
	double temp_data_1[TD_SIZE_1];
	double temp_data_2[TD_SIZE_2];
	int tDcounter = 0;
	int tScounter = 0;
	...
	if(playbackFile.is_open()){
	  ...
	  cout<<"Unable to open playbackFile. File open is: "<<playbackFile.is_open()<<endl;
	...
	// Get Temp Data 1
	ifstream tempFile;
	tempFile.open(TEMP_FILE_1);
	if(tempFile.is_open()){
	  for (int i = 0; i != TD_SIZE_1; i++) 
	    tempFile >> temp_data_1[i];
	}
	else{
	  cout<<"Unable to open playbackFiletemplate file. File open is: "<<tempFile.is_open()<<endl;
	  if (getcwd(cwd, sizeof(cwd)) != NULL) {
	    cout<<"Current working pback dir: "<<cwd<<endl;
	  }
	}
	...
	// Get Temp Data 2
	//ifstream tempFile;
	tempFile.open(TEMP_FILE_2);
	if(tempFile.is_open()){
	  for (int i = 0; i != TD_SIZE_2; i++) 
	    tempFile >> temp_data_2[i];
	}
	else{
	  cout<<"Unable to open playbackFiletemplate file. File open is: "<<tempFile.is_open()<<endl;
	  if (getcwd(cwd, sizeof(cwd)) != NULL) {
	...
	double columnMean_1;
	double mean_1 = 0;
	for (tDcounter = FFT_ARRAY_SIZE; tDcounter != TD_SIZE_1; tDcounter++)
		if (( (tDcounter - FFT_ARRAY_SIZE) % (SAMP_FREQ / (1000000 / FFT_PERIOD) ) == 0)) {
			FftTemp_1(temp_data_1, tDcounter, tScounter, columnMean_1);
			mean_1 = mean_1 + columnMean_1;
		}

	// Subtract the Mean
	mean_1 = mean_1 / SPECT_ARRAY_SIZE_1;
	for (int i = 0; i != SPECT_ARRAY_SIZE_1; i++)
		for (int j = BEGIN_INDEX;  (j != FFT_ARRAY_SIZE / 2) && (j != CUTOFF_INDEX); j++)
			temp_spect_1[i][j] = temp_spect_1[i][j] - mean_1;

	// Calculate Template Length
	tempLen_1 = 0;
	for (int i = 0; i != SPECT_ARRAY_SIZE_1; i++)
		for (int j = BEGIN_INDEX;   (j != FFT_ARRAY_SIZE / 2) && (j != CUTOFF_INDEX); j++)
			tempLen_1 = tempLen_1 + pow(temp_spect_1[i][j],2);
	tempLen_1 = sqrt(tempLen_1);

	// Normalize Template
	for (int i = 0; i != SPECT_ARRAY_SIZE_1; i++)
		for (int j = BEGIN_INDEX;   (j != FFT_ARRAY_SIZE / 2) && (j != CUTOFF_INDEX); j++)
			temp_spect_1[i][j] = temp_spect_1[i][j] / tempLen_1;
	...
	double columnMean_2;
	double mean_2 = 0;
	for (tDcounter = FFT_ARRAY_SIZE; tDcounter != TD_SIZE_2; tDcounter++)
		if (( (tDcounter - FFT_ARRAY_SIZE) % (SAMP_FREQ / (1000000 / FFT_PERIOD) ) == 0)) {
			FftTemp_2(temp_data_2, tDcounter, tScounter, columnMean_2);
			mean_2 = mean_2 + columnMean_2;
		}

	// Subtract the Mean
	mean_2 = mean_2 / SPECT_ARRAY_SIZE_2;
	for (int i = 0; i != SPECT_ARRAY_SIZE_2; i++)
		for (int j = BEGIN_INDEX;  (j != FFT_ARRAY_SIZE / 2) && (j != CUTOFF_INDEX); j++)
			temp_spect_2[i][j] = temp_spect_2[i][j] - mean_2;

	// Calculate Template Length
	tempLen_2 = 0;
	for (int i = 0; i != SPECT_ARRAY_SIZE_2; i++)
		for (int j = BEGIN_INDEX;   (j != FFT_ARRAY_SIZE / 2) && (j != CUTOFF_INDEX); j++)
			tempLen_2 = tempLen_2 + pow(temp_spect_2[i][j],2);
	tempLen_2 = sqrt(tempLen_2);

	// Normalize Template
	for (int i = 0; i != SPECT_ARRAY_SIZE_2; i++)
		for (int j = BEGIN_INDEX;   (j != FFT_ARRAY_SIZE / 2) && (j != CUTOFF_INDEX); j++)
			temp_spect_2[i][j] = temp_spect_2[i][j] / tempLen_2;
...
// FFT the Template
void FftTemp_1(double temp_data[], int tDcounter, int& tScounter, double& mean)
{
	// Fill FFT-In Array
	...
	for (int i = BEGIN_INDEX; (i != HALF_FFT) && (i != CUTOFF_INDEX); i++) 
		temp_spect_1[tScounter][i] = transformedFFT[i];
	tScounter++;
}


// FFT the Template
void FftTemp_2(double temp_data[], int tDcounter, int& tScounter, double& mean)
{
	// Fill FFT-In Array
	double fftBuffer[FFT_ARRAY_SIZE];
	for (int i = 0; i != FFT_ARRAY_SIZE; i++)
	...
	// Fill Spectrogram Array
	for (int i = BEGIN_INDEX; (i != HALF_FFT) && (i != CUTOFF_INDEX); i++) 
		temp_spect_2[tScounter][i] = transformedFFT[i];
	tScounter++;
}
	...
	if (rmsHit) {
	  if ((record_counter++ == RECORD_MAX)) {
	    if ((RmsValue(POINTS_IN_RMS_TERM) < RMS_TERM_THRES)) {
	      ...
	      record_counter = 0;
	      writeData = true;
	      cout<<"RMS must here be lower than RMS_THRESH: "<<RmsValue(POINTS_IN_RMS_TERM)<<endl;
...
void Compare_Spects()
{
	...
	if(which_temp==1){
	  for (int i = sBcounter; i != SPECT_ARRAY_SIZE; i++, k++)
	    for (int j = BEGIN_INDEX; (j != FFT_ARRAY_SIZE / 2) && (j != CUTOFF_INDEX); j++)
	      correlation_coefficient = correlation_coefficient + currentSpect[i][j] * temp_spect_1[k][j];
	  ...
	  cout<<"correlation_coefficient 1 : "<<correlation_coefficient<<endl;


	  // Switch to temp_2
	  if (correlation_coefficient >= corr_coeff_trig_1) {
	    which_temp=2;
	    //isInteresting = true;
	    //zapEm = true;
	  }
	  ...
	  for (int i = sBcounter; i != SPECT_ARRAY_SIZE; i++, k++)
	    for (int j = BEGIN_INDEX; (j != FFT_ARRAY_SIZE / 2) && (j != CUTOFF_INDEX); j++)
	      correlation_coefficient = correlation_coefficient + currentSpect[i][j] * temp_spect_2[k][j];
	  for (int i = 0; k != SPECT_ARRAY_SIZE; i++, k++)
	    for (int j = BEGIN_INDEX; (j != FFT_ARRAY_SIZE / 2) && (j != CUTOFF_INDEX); j++)
	      correlation_coefficient = correlation_coefficient + currentSpect[i][j] * temp_spect_2[k][j];
	  ...
	  // Normalize 
	  correlation_coefficient = correlation_coefficient / spectLen;
	  cout<<"correlation_coefficient 2 : "<<correlation_coefficient<<endl;

	  // Zap Em? (output)
	  if (correlation_coefficient >= corr_coeff_trig_2 && deadCounter == 0) {
	    isInteresting = true;
	    zapEm = true;
	    which_temp=1;
	  }
...
void StopStart()
{
  ...
  // DataRecorder::Panel::stopRecording(0);
  // DataRecorder::Panel::startRecording(0);
  isInteresting = false;
}
...
void DataRecorder::Panel::execute(void) { 
   if(recording && !counter++) {
	...
	if (writeData) {
	  ...
	  // //Doing this writes only one time. "Trial number Complete" pops up only once
	  // //DataRecorder::Panel::stopRecording(0);
	  // //DataRecorder::Panel::startRecording(0);
	  // //isInteresting = false;
	  // }
	  ...
	  //cout<<"is file open: "<<file.is_open()<<endl;
	  ...
	  if(file.is_open()){
	    ...
	    cout<<"Unable to open file. File is open:  "<<file.is_open()<<endl;
	    if (getcwd(cwd, sizeof(cwd)) != NULL) {
...
int DataRecorder::Panel::startRecording(long long timestamp)
{
    ...
    //Added :: to ispunct at line 2071 below so that ispunct is taken from the main namespace instead of std namespace.
    size_t count = 0;
    for (RT::List<Channel>::iterator i = channels.begin(), end = channels.end(); i != end; ++i)
        {
