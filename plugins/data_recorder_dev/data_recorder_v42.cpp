...
// Parameters: Trigger
double corr_coeff_trig = 0.79;//0.74;		// Correlation to Template needed for Detection
...
void DataRecorder::Panel::execute(void) { 
   if(recording && !counter++) {
	...
	// if(sec_counter==max_sec_counter){
	...
	if (writeData) {
	  ...
	  //try to write data as in v0.
			...
			//Put data into fifo
			token.type = SYNC;
			for (int i = 0; i != (CB_ARRAY_SIZE-1); i++) { //The data point at predata[0][CB_ARRAY_SIZE] is contained in dataStorage[0][0]
			  appendData[0] = predata[0][i];
			  //appendData[1] = predata[1][i];
