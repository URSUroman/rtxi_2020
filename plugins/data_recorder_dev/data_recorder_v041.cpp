...
long int max_counter = 40000;
long int h5_counter = 0;
		...
		if (state == RECORD)
		  {
		    ...
		    std::cout<<"counter: "<<h5_counter<<std::endl;
		    if(h5_counter == max_counter){ //each second write data from fifo to H5 file
			...
			continue; // Restart loop if data is not available
		      ...
		      std::cout<<"token (should be current SYNC): "<<_token.type<<std::endl;
		      std::cout<<"data[0] (current): "<< data[0] <<std::endl;
		      fifo.read(&_token, sizeof(_token));
		      std::cout<<"token just read after first SYNC: "<<_token.type<<std::endl;
		      fifo.read(data, _token.size);
